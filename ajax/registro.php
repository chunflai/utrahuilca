...
$idhoja=isset($_POST["idhoja"])? limpiarCadena($_POST["idhoja"]):"";
...
switch ($_GET["op"]){
    case 'GuardaryEditar':
        if (empty($idhoja)){
            $rspta=$Registro->insertar($idvacante,$idDepartamentosNacimiento,$idciudadesNacimiento,$idescolaridad,$idtitulacion,$idDepartamentoResidencia,$idCiudadesResidencia,$idDeparmentoFormacion,$idciudadFormacion,$idDepartamentoTrabajo,$idCiudadTrabajo,$politicas_Aceptacion,$Nombres,$Apellidos,$TipoIdentificacion,$NumeroIdentificacion,$genero,$estadoCivil,$FechaNacimiento,$Direccion,$barrio,$Telefono,$Celular,$CorreoElectronico,$estadoEstudio,$FechaInicio,$FechaFin,$TituloOtorgado,$NombreInstitucion,$cursoNombTitulo,$NomIntitu,$fechainc,$fechafi,$Empresa,$FechaIni,$Telefo,$EstadoTrabajo,$FechaFinalizacion,$jefeInmediato,$NombreCargo,$FuncionesPrincipales,$empresa2,$jefeInmediato2,$fechaini2,$fechater2,$telefon2,$FuncionesPrin2,$nombrecargo2,$NombreFa,$TelefonoFa,$celufa,$Parentesco,$NombrePersonal1,$Telefono1,$Celul,$NombrePersonal2,$Telefono2,$cel);
            echo $rspta ? "Registro registrada" : "Registro no se pudo registrar";
        }
        else {
            $rspta=$Registro->editar($idhoja,$idvacante,$idDepartamentosNacimiento,$idciudadesNacimiento,$idescolaridad,$idtitulacion,$idDepartamentoResidencia,$idCiudadesResidencia,$idDeparmentoFormacion,$idciudadFormacion,$idDepartamentoTrabajo,$idCiudadTrabajo,$politicas_Aceptacion,$Nombres,$Apellidos,$TipoIdentificacion,$NumeroIdentificacion,$genero,$estadoCivil,$FechaNacimiento,$Direccion,$barrio,$Telefono,$Celular,$CorreoElectronico,$estadoEstudio,$FechaInicio,$FechaFin,$TituloOtorgado,$NombreInstitucion,$cursoNombTitulo,$NomIntitu,$fechainc,$fechafi,$Empresa,$FechaIni,$Telefo,$EstadoTrabajo,$FechaFinalizacion,$jefeInmediato,$NombreCargo,$FuncionesPrincipales,$empresa2,$jefeInmediato2,$fechaini2,$fechater2,$telefon2,$FuncionesPrin2,$nombrecargo2,$NombreFa,$TelefonoFa,$celufa,$Parentesco,$NombrePersonal1,$Telefono1,$Celul,$NombrePersonal2,$Telefono2,$cel);
            echo $rspta ? "Registro actualizada" : "Registro no se pudo actualizar";
        }
    break;

    case 'desactivar':
        $rspta=$Registro->desactivar($idhoja);
        echo $rspta ? "Registro Desactivada" : "Registro no se puede desactivar";
        break;


    case 'activar':
        $rspta=$Registro->activar($idhoja);
        echo $rspta ? "Registro activada" : "Registro no se puede activar";
        break;


    case 'Mostrar':
        $rspta=$Registro->mostrar($idhoja);
    ...
    case 'Listar':
        ...
        while ($reg=$rspta->fetch_object()){
            $data[]=array(
                "0"=>($reg->estado)?'<button class="btn btn-warning" onclick="mostrar('.$reg->idhoja.')"><i class="fa fa-pencil"></i></button>'.
                    ' <button class="btn btn-danger" onclick="desactivar('.$reg->idhoja.')"><i class="fa fa-close"></i></button>':
                    '<button class="btn btn-warning" onclick="mostrar('.$reg->idhoja.')"><i class="fa fa-pencil"></i></button>'.
                    ' <button class="btn btn-primary" onclick="activar('.$reg->idhoja.')"><i class="fa fa-check"></i></button>',
                ...
                "13"=>$reg->barrio,
                "14"=>$reg->Telefono,
                "15"=>$reg->idDepartamentoResidencia,
                "16"=>$reg->idCiudadesResidencia,
                "17"=>$reg->Celular,
                ...
                "36"=>$reg->idDepartamentoTrabajo,
                "37"=>$reg->idCiudadTrabajo,
                "38"=>$reg->FechaFinalizacion,
