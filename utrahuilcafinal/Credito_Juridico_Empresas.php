...
$sql="SELECT * FROM `linea_credito` WHERE tipo = 'CJ' and codigo=".$codigo_cred;
$result=mysqli_query($conexion,$sql);
while ($mostrar=mysqli_fetch_array($result)) {
	?>
...
<?php 
$sql="SELECT * FROM `linea_credito` WHERE tipo = 'CJ' order by rand() limit 0, 5";
$result=mysqli_query($conexion,$sql);
while ($mostrar=mysqli_fetch_array($result)) {
	?>
<li class="itemcred">
    <a class="nav-link" href="Credito_Juridico_Empresas.php?tabPag=<?php  echo $mostrar['codigo'];?>" role="tab" aria-controls="home" id="scroll"><img src="images/<?php  echo $mostrar['imagen'];?>" style="width: 50px"><br><?php  echo $mostrar['nombre'];?></a>
  </li>
  <?php }  ?>
  </ul>
</div>
